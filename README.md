...
1. **Describe the image** through a Vision Language Model (Anthropic
   Claude, OpenAI GPT-4o, or Google Gemini), producing a structured
   description (object class, colours, brand, distinguishing marks).
...
| --- | --- |
| **Vision understanding** | Anthropic Claude · OpenAI GPT-4o · Google Gemini |
...
```bash
cp .env.example .env
# Open .env and provide at least one provider key:
...
| --- | --- | --- |
| `LLM_PROVIDER` | `anthropic` | Primary VLM provider (`anthropic`, `openai`, `gemini`) |
| `LLM_MODEL` | `claude-sonnet-4-6` | Model identifier passed to the provider |
| `ANTHROPIC_API_KEY` | — | Anthropic API key |
| `OPENAI_API_KEY` | — | OpenAI API key |
| `GOOGLE_API_KEY` | — | Google / Gemini API key |
| `EMBEDDING_PROVIDER` | `openai` | Embedding provider |
| `EMBEDDING_MODEL` | `text-embedding-3-small` | Embedding model |
| `SECONDARY_LLM_PROVIDER` | — | Optional failover VLM provider |
| `SECONDARY_LLM_MODEL` | — | Optional failover VLM model |
...
│   └── demo_ai.py         Provided sample harness
│
├── docs/                  Architecture, plan, templates
├── data/samples/          Sample lost/found images (committed)
...
├── PROJECT_STATUS.md      Build status and continuity contract
├── PROJECT_STRUCTURE.md   Directory rationale
└── TEAM_GIT_WORKFLOW_PLAN.md  Team collaboration workflow
```

A more detailed walkthrough of why the structure is laid out this way
lives in [`PROJECT_STRUCTURE.md`](PROJECT_STRUCTURE.md).
...
- Anthropic SDK · OpenAI SDK · Google Generative AI SDK (via the
  provided `ai/` library)
...
Detailed contribution attribution per source file is recorded in
[`docs/templates/CONTRIBUTION_STATEMENT.md`](docs/templates/CONTRIBUTION_STATEMENT.md)
and signed by all three members.

The collaborative workflow itself — branch naming, commit convention,
the 33-PR sequence, review assignments, conflict resolution — is fully
documented in [`TEAM_GIT_WORKFLOW_PLAN.md`](TEAM_GIT_WORKFLOW_PLAN.md).
