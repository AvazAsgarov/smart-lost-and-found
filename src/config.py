...
class Settings(BaseSettings):
    ...
    so the application boots in development mode (SQLite, Anthropic VLM,
    OpenAI embeddings) without further configuration. Production deployments
    override values via environment variables or a mounted ``.env`` file.
    ...
    LLM_MODEL: str = Field("claude-sonnet-4-6", description="Provider-specific model ID")
    ...
    SECONDARY_LLM_PROVIDER: str | None = Field(None, description="Fallback VLM provider")
